...
namespace Codaxy.WkHtmlToPdf
{
    ...
    public class PdfConvert
    {
        ...
        public static void ConvertHtmlToPdf(PdfDocument document, PdfOutput output, Encoding encoding, string orientation = null, string pageSize = "A4")
        {
            ConvertHtmlToPdf(document, null, output, encoding,  orientation, pageSize);
        }

        public static void ConvertHtmlToPdf(PdfDocument document, PdfConvertEnvironment environment, PdfOutput woutput, Encoding encoding, string orientation, string pageSize)
        {
            if (environment == null)
            ...
            paramsBuilder.Append($@"--page-size {pageSize} ");
            if(!string.IsNullOrEmpty(orientation))
                paramsBuilder.Append(orientation);
                    ...
                    using (AutoResetEvent errorWaitHandle = new AutoResetEvent(false))
                    {
                        ...
                        try
                        {
                            ...
                            if (document.Html != null)
                            {
                                ...
                                {
                                    byte[] buffer = encoding.GetBytes(document.Html);
        ...
        internal static void ConvertHtmlToPdf(string url, string outputFilePath, Encoding encoding)
        {
            ConvertHtmlToPdf(new PdfDocument { Url = url }, new PdfOutput { OutputFilePath = outputFilePath }, encoding);
        }
    }
