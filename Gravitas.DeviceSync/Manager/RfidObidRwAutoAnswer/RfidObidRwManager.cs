...
namespace Gravitas.Core.Manager {
	public class RfidObidRwManager : IRfidObidRwManager {
		...
		public RfidObidRwManager(DeviceRepository deviceRepository, long deviceId) {
			_deviceRepository = deviceRepository;
			_deviceId = deviceId;
		}

		public void GetInformationAsync(IPAddress ipAddress, int port, CancellationToken token) {
				...
				try {
					using (TcpClient tcpClient = new TcpClient()) {
						...
						while (true) {
							...
							if (rfid != null) {
								UpdateDeviceState(0, new RfidObidRwInJsonState() {Rifd = rfid});
							}
							else {
								UpdateDeviceState(255, new RfidObidRwInJsonState() {Rifd = string.Empty});
							}
...
//					Logger.Debug($@"Device: {_deviceId}. IOException.", e.ToString());
		...
		private string Parse(byte[] dataBytes, int bytesCount) {
			...
			if (bytesCount < dataLen) {
				Logger.Warn($@"Device: {_deviceId}. Not enought data was received.");
				return null;
			}
			...
			Logger.Info($@"Device: {_deviceId}. Card = {cardRfid}, ChSum = {chSum:X2}.");
