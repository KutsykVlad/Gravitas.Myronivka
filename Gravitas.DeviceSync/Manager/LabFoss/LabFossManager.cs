...
namespace Gravitas.DeviceSync.Manager {
	...
	class LabFossManager : ILabFossManager {

		protected static class DataIndex {
		...
		public void SyncData() {
			...
			_dirWatcher.Path = param.Directory;
			_dirWatcher.NotifyFilter = NotifyFilters.LastWrite;
			_dirWatcher.Filter = "*.csv";
		...
		private void OnChanged(object source, FileSystemEventArgs e) {
			try {
				string[] data = FileHelper.ReadAllLines(e.FullPath, 5)?.Skip(1)?.FirstOrDefault()?.Split(';');
				...
				LabFossInJsonState fossInJsonState = new LabFossInJsonState {
					AnalysisTime = DateTime.ParseExact(data[DataIndex.AnalysisTime], new [] { "dd.MM.yyyy H:mm:ss", "dd.MM.yyyy HH:mm:ss" },  CultureInfo.InvariantCulture, DateTimeStyles.None),
